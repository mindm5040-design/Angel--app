...
Fix: session_state non accessible depuis les threads + suppression Gemini.
"""
...
# ------------------------------------------------------------------
# ÉTAT DE SESSION - INITIALISATION ROBUSTE
# ------------------------------------------------------------------
defaults = {
...
# Nettoyage ancienne clé gemini si elle existe encore
...
def fuse_answer(question):
    kC, kO = get_keys()
    system_prompt = build_system_prompt() # IMPORTANT: construit ici, pas dans les threads

    jobs = {}
    with ThreadPoolExecutor(max_workers=2) as pool:
        if kC:
            jobs[pool.submit(call_claude, question, kC, system_prompt)] = "CLAUDE"
        if kO:
            jobs[pool.submit(call_openai, question, kO, system_prompt)] = "GPT"
...
def render_sidebar():
    with st.sidebar:
        ...
        with st.expander("🔑 Clés API & moteurs"):
            st.caption("Clés stockées en session uniquement.")
...
def render_chat():
    ...
    st.caption("CLAUDE · GPT — fusion active · ton direct, méthodique, sans détour")
...
def handle_question(question):
    ...
    with st.spinner("Fusion en cours — Claude · GPT…"):
        try:
